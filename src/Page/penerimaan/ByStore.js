...
import ModalAddPenerimaanStorekeStore from "../../Component/modal/Modal-AddPenerimaanStorekeStore-Component";
import ModalEditPenerimaanStorekeStore from "../../Component/modal/Modal-EditPenerimaanStorekeStore-Component";
...
import {
  geReportPenerimaanFromStore,
  getOffice,
  getUkuran,
  addPenenrimaanStore,
  getStore,
  getPenenrimaanStore,
  getPenenrimaanStoreSearch,
  updatePenenrimaanStore,
  deletePenenrimaanStore,
} from "../../Config/Api-new";
...
export default function PengirimanStoreStore() {
  ...
  const submitPengirimanStorekeStor = async (
    ...
    let res = await addPenenrimaanStore(
      detail_pengiriman,
      tanggal_pengiriman,
      id_store_asal,
      lokasi_store_asal,
    ...
    if (idx > -1) {
      for (let i = 0; i < array?.length; i++) {
        if (array[i]?.check === true) {
          await deletePenenrimaanStore(array[i]?.id);
        }
      }
      getAllKategori();
      setCheck(!check);
  ...
  const getAllKategori = async () => {
    let res = await getPenenrimaanStore();
  ...
  const searching = async (e, type) => {
    if ((type === "enter" && e.keyCode === 13) || type === "klik") {
      let res = await getPenenrimaanStoreSearch(searched);
      setData(res?.data);
      setPage(0);
      setRowsPerPage(5);
    }
  ...
  return (
    ...
    >
      <div style={{ display: "flex" }}>
        <h1>Penerimaan Store from Store</h1>
        <div
          style={{
            position: "absolute",
            right: 0,
      ...
      />
      <ModalAddPenerimaanStorekeStore
        open={modal}
        store={dataToko}
        ukuran={ukuran}
        submit={(
