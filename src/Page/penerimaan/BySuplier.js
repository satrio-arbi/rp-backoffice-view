...
import ModalPenerimaanAddSupplier from "../../Component/modal/Modal-AddPenerimaSupplier-Component";
import ModalEditSupplier from "../../Component/modal/Modal-EditPenerimaSupplier-Component";
...
import {
  geReportPenerimaanFromSupplier,
  getOffice,
  getUkuran,
  addPenenrimaanSuplier,
  getStore,
  getPenenrimaanSuplier,
  getPenenrimaanSuplierSearch,
  updatePenenrimaanSuplier,
  deletePenenrimaanSuplier,
} from "../../Config/Api-new";
...
export default function PengirimanStoreStore() {
  ...
  const submitPengirimanStorekeStor = async (
    ...
    let res = await addPenenrimaanSuplier(
      detail_pengiriman,
      tanggal_pengiriman,
      id_supplier,
      nama_supplier,
    ...
    if (idx > -1) {
      for (let i = 0; i < array?.length; i++) {
        if (array[i]?.check === true) {
          await deletePenenrimaanSuplier(array[i]?.id);
        }
      }
      getAllKategori();
      setCheck(!check);
  ...
  const submitUpdateKategori = async (
    ...
    let res = await updatePenenrimaanSuplier(
      detail_pengiriman,
      tanggal_pengiriman,
      id_supplier,
      nama_supplier,
      pengiriman,
      toBeSelected?.penerimaan_code,
      toBeSelected?.id
    );
    if (res?.status) {
      alertSuccess("Success", "");
  ...
  const getAllKategori = async () => {
    let res = await getPenenrimaanSuplier();
  ...
  const searching = async (e, type) => {
    if ((type === "enter" && e.keyCode === 13) || type === "klik") {
      let res = await getPenenrimaanSuplierSearch(searched);
      setData(res?.data);
      setPage(0);
      setRowsPerPage(5);
    }
