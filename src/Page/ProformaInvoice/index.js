...
import ModalAddProformaInvoice from "../../Component/modal/Modal-AddProformaInvoice-Component";
import ModalUpdateProformaInvoice from "../../Component/modal/Modal-UpdateProformaInvoice-Component";
...
import {
  ...
  getUkuran,
  getDownloadProformaInvoice,
  getPelanggan,
  getKategori,
  getTipe,
  getProformaInvoice,
  getProformaInvoiceSearch,
  getProformaInvoiceAdd,
  getProformaInvoiceUpdate,
  getProformaInvoiceDelete,
} from "../../Config/Api-new";
...
export default function ProformaInvoice() {
  ...
  const dataStore = useSelector(
    (state) => state.reducer.getProformaInvoice?.data
  );
  ...
  const submitProformaInvoice = async (v) => {
    setModal(false);
    let res = await getProformaInvoiceAdd(v);
    if (res?.status) {
    ...
    if (idx > -1) {
      for (let i = 0; i < array?.length; i++) {
        if (array[i]?.check === true) {
          await getProformaInvoiceDelete(parseInt(array[i]?.id));
        }
      }
      getAllProformaInvoice();
      setCheck(!check);
  ...
  const submitUpdateProformaInvoice = async (v) => {
    setOpenDetail(false);
    settoBeSelected({});
    let arr = {
      detail_proforma_invoice: v?.detail_proforma_invoice,
    ...
    let res = await getProformaInvoiceUpdate(arr);
    if (res?.status) {
      alertSuccess("Success", "");
      getAllProformaInvoice();
      downloadInvoice(res?.data);
  ...
  const getAllProformaInvoice = async () => {
    let res = await getProformaInvoice();
  ...
  const searching = async (e, type) => {
    if ((type === "enter" && e.keyCode === 13) || type === "klik") {
      let res = await getProformaInvoiceSearch(searched);
      setData(res?.data);
    }
  };
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <ModalUpdateProformaInvoice
        ...
        ukuran={ukuran}
        karyawan={karyawan}
        submit={(v) => submitUpdateProformaInvoice(v)}
        onClickOpen={() => setOpenDetail(!openDetail)}
      />
      <ModalAddProformaInvoice
        open={modal}
        bank={bank}
        pelanggan={pelanggan}
        office={office}
        ...
        ukuran={ukuran}
        karyawan={karyawan}
        submit={(v) => submitProformaInvoice(v)}
